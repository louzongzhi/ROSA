...
struct RosaGssConfig {
    size_t reservoir_cap;
    size_t suffix_weight;
    size_t walk_depth;

    RosaGssConfig() : reservoir_cap(16), suffix_weight(2), walk_depth(1) {}
};
...
struct RosaGssState {
    ...
    RosaGssState() : length(0), suffix_link(kInvalidIndex), total_occurrences(0),
                     write_ptr(0) {}
            ...
            if (global_zone_size > 0) {
                if (reservoir.size() < recency_zone + global_zone_size) {
                    ...
                    std::uniform_int_distribution<size_t> dist(
                        0, total_occurrences - 1 - recency_zone);
                    size_t k = dist(rng);
...
class RosaGss {
public:
    RosaGss() : last_k_(0), last_q_(0) {}

    RosaGss(const RosaGssConfig& config)
        : last_k_(0), last_q_(0), config_(config) {}

    V append(K q, K k, V v, V u) {
        P j = update_query_(q, last_q_);
        update_key_value_(k, v, rng_);
        return j != kInvalidIndex ? values_[j + 1] : u;
    }

    V append_with_sampling(K q, K k, V v, V u, std::vector<P>& indices,
                          std::vector<F>& quality, size_t num_samples, F tau,
                          size_t index_offset) {
        P j = random_walk_query_(indices, quality, q, last_q_, num_samples, tau,
                                rng_, index_offset);
        update_key_value_(k, v, rng_);
        return j != kInvalidIndex ? values_[j + 1] : u;
    }
...
private:
    ...
    std::vector<RosaGssState<K, V, P>> states_;

    P last_k_;
    P last_q_;

    std::mt19937 rng_;
    RosaGssConfig config_;

    P random_walk_query_(std::vector<P>& indices, std::vector<F>& quality, K q,
                        P& s, size_t num_samples, F tau, std::mt19937& rng,
                        size_t index_offset) {
        if (states_.empty()) states_.emplace_back();

        std::map<P, P> candidates;

        // r: current node during walk
        P r = s;
        P cur_depth = 0;
        P walk_dist = 0;

        while (r != kInvalidIndex && cur_depth < config_.walk_depth) {
            for (auto it = states_[r].transitions.begin();
                 it != states_[r].transitions.end(); ++it) {
        ...
        if (candidates.size() <= num_samples) {
            ...
            std::partial_sort(scores.begin(), scores.begin() + num_samples,
                              scores.end(), std::greater<>());
            for (size_t k = 0; k < num_samples; ++k) {
                auto it = candidates.find(scores[k].second);
                states_[it->first].update_endpos_table(duplicated_endpos,
                                                        it->second);
            }
        }
        ...
        for (auto it = duplicated_endpos.begin(); it != duplicated_endpos.end();
             ++it) {
    ...
    P update_query_(K q, P& s) {
        ...
        // r: current node during traversal
        P r = s;
        while (r != kInvalidIndex && !states_[r].transitions.count(q)) {
            r = states_[r].suffix_link;
        }
        ...
        if (r == kInvalidIndex) {
            ...
            while (r != kInvalidIndex) {
                if (states_[r].length > 0 &&
                    states_[r].find_query_endpos(j, config_.reservoir_cap)) {
                    break;
                }
                r = states_[r].suffix_link;
            }
...
torch_rosa_gss_forward(const torch::Tensor& q, const torch::Tensor& k,
                       const torch::Tensor& v, int64_t u, int64_t num_samples,
                       double tau) {
    int64_t B = q.size(0);
    int64_t N = q.size(1);
    ...
    for (int64_t i = 0; i < B; ++i) {
        ...
        for (int64_t t = 0; t < N; ++t) {
            ...
            out_a[i][t] = r.append_with_sampling(
                q_a[i][t], k_a[i][t], v_a[i][t], static_cast<V>(u), indices, quality,
                num_samples, tau, i * N);
